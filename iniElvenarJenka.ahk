;mennyi van belőle
erdeiGyartoSorNum := 0
erdeiGyartoSor2Num := 0
erdeiMarvanyNum := 0
erdeiAcelNum := 0
erdeiFaNum := 0
kristalyNum := 6
marvanyNum := 2
marvany2Num := 2
marvany3Num := 0
marvany4Num := 0
dragakoNum := 4
szerszam1Num := 4
szerszam2Num := 5
szerszam3Num := 3


;koordináták és különbségek
erdeiGyartoSorX := 1156
erdeiGyartoSorY := 739
erdeiGyartoSorDefX := -63
erdeiGyartoSorDefY := -33
erdeiGyartoSor2X := 762
erdeiGyartoSor2Y := 631
erdeiMarvanyX := 955
erdeiMarvanyY := 718
erdeiMarvanyDefX := -30
erdeiMarvanyDefY := -16
erdeiAcelX := 720
erdeiAcelY := 642
erdeiAcelDefX := 61
erdeiAcelDefY := 33
erdeiFaX := 762
erdeiFaY := 619
erdeiFaDefX := 61
erdeiFaDefY := 33

kristalyX := 692
kristalyY := 442
kristalyDefX := -65
kristalyDefY := -32
dragakoX := 549
dragakoY := 444
dragakoDefX := -76
dragakoDefY := -40
marvanyX := 851
marvanyY := 537
marvany2X := 791
marvany2Y := 560
marvany3X := 711
marvany3Y := 503
marvany4X := 582
marvany4Y := 556
marvanyDefX := -96
marvanyDefY := -48
torpVedoBastyaX := 506
torpVedoBastyaY := 232
torpVedoBastyaDefX := 80 ;ez azért kell, mert ebből számolja ki a megengedett eltérést (ennek az 5%-a)
torpVedoBastyaDefY := 60
gombakSzentelyeX := 939
gombakSzentelyeY := 587
gombakSzentelyeDefX := 60 ;ez azért kell, mert ebből számolja ki a megengedett eltérést (ennek az 5%-a)
gombakSzentelyeDefY := 40
aranySzakadekX := 1294
aranySzakadekY := 419
aranySzakadekDefX := 60 ;ez azért kell, mert ebből számolja ki a megengedett eltérést (ennek az 5%-a)
aranySzakadekDefY := 60
szerszam1X := 986
szerszam1Y := 511
szerszam1DefX := -47
szerszam1DefY := -24
szerszam2X := 634
szerszam2Y := 595
szerszam2DefX := -48
szerszam2DefY := -27
szerszam3X := 332
szerszam3Y := 411
szerszam3DefX := -61
szerszam3DefY := -30



;termelés indít:
/* 

global aruBeszed1X := 
global aruBeszed1Y := 
global aruBeszed2X := 
global aruBeszed2Y := 
global aruBeszed3X := 
global aruBeszed3Y :=
global aruBeszed4X := 
global aruBeszed4Y :=  
global szerszamBeszed1X :=
global szerszamBeszed1Y :=
global szerszamBeszed2X :=
global szerszamBeszed2Y :=
global szerszamBeszed3X :=
global szerszamBeszed3Y :=
global szerszamBeszed4X :=
global szerszamBeszed4Y :=
global szerszamBeszed5X :=
global szerszamBeszed5Y :=
global szerszamBeszed6X :=
global szerszamBeszed6Y :=
 */